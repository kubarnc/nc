...
def list_group_chats(user_id, storage_state, username, password, max_groups=10, last_n_threads=10):
    ...
    cl = Client()
    updated = False
    new_state = None  # placeholder for compatibility
    ...
    try:
        ...
        import time, random
        time.sleep(random.uniform(1.0, 3.0))  # 1–3 seconds random delay
        ...
        updated = True
        threads = cl.direct_threads(amount=last_n_threads)
        # --- Safe delay after re-login fetch ---
    ...
    for thread in threads:
        if len(groups) >= max_groups:
            break
        # Only include group chats (2+ users)
    ...
    # Return updated playwright state if session was refreshed
...
APP = None
LOOP = None
SESSIONS = {}  # user_id -> {"pid": pid, "master_fd": fd, "thread": thread, "username": username, "password": password, "secret": secret, "chat_id": chat_id}
    ...
    while True:
        try:
            ...
            while b"\n" in buf or len(buf) > 2048:
                ...
                if not text:
                    continue
                # Filter repeated echoes like Code entered
                if text.startswith("Code entered"):
                    continue
                # ✅ Skip unnecessary raw or debug logs
                lower = text.lower()
                if (
                    len(text) > 300
...
async def progress_edit(bot, chat_id, msg_id, pid, gc_display):
    start_time = time.time()
    while time.time() - start_time < 7:
        elapsed = time.time() - start_time
        perc = int((elapsed / 7) * 100) + 1
        if perc > 100:
            perc = 100
        text = f"💥 Changing GC name! To stop this task type /stop {pid} 💥\nProgress: {perc}%"
        try:
            await bot.edit_message_text(chat_id=chat_id, message_id=msg_id, text=text)
        except:
            pass
        await asyncio.sleep(0.35)
    await bot.send_message(chat_id=chat_id, text=f"PID NO {pid} — {gc_display}")
...
async def get_names(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for selected in selected_gcs:
        ...
        # 🟢 Start actual task subprocess (no fake progress, no edit)
        proc = subprocess.Popen(cmd)
        pid = proc.pid

        # 3️⃣ Save task info
        task = {'id': task_id, 'proc': proc, 'pid': pid, 'thread_url': thread_url, 'account': username, 'gc_display': selected['display'], 'start_time': time.time()}
        tasks.append(task)
        logging.info(f"{time.strftime('%Y-%m-%d %H:%M:%S')} Task start user={user_id} task={task_id} url={thread_url} cmd={' '.join(cmd)}")

        # 4️⃣ Send only one message — no edits later
...
async def task_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users_tasks or not users_tasks[user_id]:
        ...
        return
    tasks = users_tasks[user_id]
    active_tasks = [t for t in tasks if t['proc'].poll() is None]
